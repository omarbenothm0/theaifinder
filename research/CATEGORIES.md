...
CATEGORY: Coding & Software Development | Slug: `coding` | Seed overlap: REPLACE — enriched seed placeholder `cat-coding` via `lib/data/coding-category.ts` | Evidence: Keyword `best ai coding tools` → `/category/coding` in KEYWORDS.md (HARD competition; raw volume NOT VERIFIED). Verified inventory: Cursor, Claude Code, v0. Developers persona remains NOINDEX — coding intent served by category, tool pages, and `claude-code-vs-cursor` comparison. | Status: IMPLEMENTED | Date: 2026-08-12
Notes: Lightweight hub with agentic vs UI-generation sections, FAQs from official sources, comparison CTA, and live tool count. Does not revive `/for/developers` or add unverified tools.
