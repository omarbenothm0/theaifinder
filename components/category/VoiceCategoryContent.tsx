...
import { VOICE_SECTIONS } from '../../lib/data/voice-category';
...
interface VoiceCategoryContentProps {
  category: Category;
  categories: Category[];
  personas: Persona[];
}
...
export async function VoiceCategoryContent({
  category,
  categories,
  personas,
}: VoiceCategoryContentProps) {
  const allSlugs = VOICE_SECTIONS.flatMap((section) => section.toolSlugs);
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-background-raised border border-accent/20 rounded-2xl p-5 space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Narration, dubbing, and voice cloning — ElevenLabs for spoken audio output, not full
            song composition with instruments.
          </p>
          <Link
            href="/category/video"
            className="inline-flex items-center gap-1.5 text-xs font-bold text-accent hover:text-foreground-strong"
          >
            Video &amp; Motion — podcast and edit workflows
        ...
        <div className="bg-background-raised border border-intent-secondary-border rounded-2xl p-5 space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Full songs from prompts — Suno AI with vocals and instrumentation, not a TTS or dubbing
            platform for narrated speech.
          </p>
          <Link
            href="/for/marketers"
            className="inline-flex items-center gap-1.5 text-xs font-bold text-intent-secondary hover:text-intent-secondary-hover"
          >
            Marketers hub — campaign audio and content
            <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto flex flex-wrap gap-3">
        ...
        >
          Descript — transcript-based audio/video editing
          <ArrowRight className="w-3.5 h-3.5" />
        </Link>
      </div>
