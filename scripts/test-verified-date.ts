...
async function main() {
  ...
  const updated = await ToolRepository.updateTool(TEST_SLUG, { tagline: created.tagline + ' Updated.' });
  ...
  const afterCheck = await ToolRepository.getToolBySlug(TEST_SLUG, { includeUnpublished: true });
